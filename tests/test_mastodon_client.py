...
class TestMastodonClient:
    ...
    def test_init(self):
        ...
        client = MastodonClient(api_base_url="https://mastodon.social", access_token="test_token")
    ...
    def test_authenticate_success(self, mock_mastodon_library):
        ...
        mock_mastodon_library.assert_called_with(access_token="test_token", api_base_url="https://mastodon.social")
    ...
    def test_post_status_success(self, mock_mastodon_library):
        ...
        mock_api = MagicMock()
        mock_api.status_post.return_value = {'id': '123', 'url': 'https://mastodon.social/@testuser/123'}
        mock_mastodon_library.return_value = mock_api
        ...
        mock_api.status_post.assert_called_with(status="Hello", in_reply_to_id="122", media_ids=["m1"])
    ...
    def test_upload_media_success(self, mock_mastodon_library):
        ...
        mock_api.media_post.assert_called_with(media_file=b"image_data", mime_type="image/jpeg", description="A test image")
    ...
    def test_get_recent_posts_success(self, mock_mastodon_library):
        ...
        mock_api.account_statuses.return_value = [
            {
                'id': 'p1', 'content': 'Post 1', 'created_at': datetime.now(), 'url': 'url1',
                'in_reply_to_id': None, 'media_attachments': []
            },
            {
                'id': 'p2', 'content': 'Reply to p1', 'created_at': datetime.now(), 'url': 'url2',
                'in_reply_to_id': 'p1', 'media_attachments': [{'id': 'm1'}]
            }
        ]
        mock_mastodon_library.return_value = mock_api
