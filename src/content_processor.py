...
class ContentProcessor:
    ...
    @staticmethod
    def process_bluesky_to_mastodon(text: str, embed: Optional[Dict[str, Any]] = None) -> str:
        """
    ...
    def _handle_embed(text: str, embed: Dict[str, Any]) -> str:
        ...
        embed_type = embed.get('py_type', '').split('.')[-1] if embed.get('py_type') else ''
    ...
    def _truncate_if_needed(text: str) -> str:
        ...
        if last_space > ContentProcessor.MASTODON_CHAR_LIMIT * 0.8:  # If we can save at least 20% by truncating at word boundary
    ...
    def add_sync_attribution(text: str, source: str = "Bluesky") -> str:
        ...
        if "(via" not in text and len(text + attribution) <= ContentProcessor.MASTODON_CHAR_LIMIT:
